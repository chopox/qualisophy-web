...
export const POST: APIRoute = async ({ request }) => {
  ...
  // 2. Server-side validation (matching your React form)
  ...
  // 3. Get the NEW webhook URL from environment variables
  const makeWebhookUrl = import.meta.env.MAKE_ENROLLMENT_WEBHOOK_URL;

  if (!makeWebhookUrl) {
    console.error("Error: MAKE_ENROLLMENT_WEBHOOK_URL is not set.");
  ...
  try {
    // 4. Send ALL form data to Make.com
    const response = await fetch(makeWebhookUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data), // Send the whole data object
    });
